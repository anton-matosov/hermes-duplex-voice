...
- Versioned `DuplexProviderAdapter`, `DuplexTransport`, `AudioSource`, and `AudioSink` interfaces.
...
- Every adapter reports ID, adapter version, accepted contract range, capabilities, and supported model/protocol range.
...
Capabilities cover transport, authentication, codecs/rates, turn detection, automatic interruption, manual cancellation, transcripts, function tools, parallel calls, resumption, rate-limit events, and mutable session fields.
...
1. Both provider adapters can satisfy one contract suite despite different transports.
