...
If neither path can prove exact scope and execution behavior, tools remain disabled and Track A may release voice without Spec 006.
...
- Backend route `POST /calls/{call_id}/tools/{tool_call_id}` reusing the minimal profile-bound call lifecycle established in Spec 002.
...
1. Backend resolves the active, unexpired application call record and the exact allowlisted tool for its bound Hermes profile/session.
...
4. Desktop submits the opaque application call ID, provider tool-call ID, name, and bounded arguments to the plugin route; the provider session/call ID remains backend-only.
...
7. Duplicate provider tool-call IDs return the stored terminal result without executing again.
8. Desktop sends `function_call_output` with the same provider tool-call ID and one `response.create` continuation.
